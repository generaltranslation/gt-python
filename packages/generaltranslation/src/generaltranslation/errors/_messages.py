...
def no_target_locale_error(fn_name: str) -> str:
    return f"{GT_ERROR_PREFIX} Cannot call `{fn_name}` without a specified locale. Either pass a locale to the `{fn_name}` function or specify a targetLocale in the GT constructor."


def no_source_locale_error(fn_name: str) -> str:
    return f"{GT_ERROR_PREFIX} Cannot call `{fn_name}` without a specified locale. Either pass a locale to the `{fn_name}` function or specify a sourceLocale in the GT constructor."


def no_project_id_error(fn_name: str) -> str:
    return f"{GT_ERROR_PREFIX} Cannot call `{fn_name}` without a specified project ID. Either pass a project ID to the `{fn_name}` function or specify a projectId in the GT constructor."


def no_api_key_error(fn_name: str) -> str:
    return f"{GT_ERROR_PREFIX} Cannot call `{fn_name}` without a specified API key. Either pass an API key to the `{fn_name}` function or specify an apiKey in the GT constructor."
