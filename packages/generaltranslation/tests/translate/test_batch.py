...
def test_create_batches_basic() -> None:
    items = list(range(5))
...
def test_create_batches_exact() -> None:
    items = list(range(4))
    result = create_batches(items, 2)
    assert result == [[0, 1], [2, 3]]


def test_create_batches_single() -> None:
    items = [1, 2, 3]
...
@pytest.mark.asyncio
async def test_process_batches_empty() -> None:
...
@pytest.mark.asyncio
async def test_process_batches_basic() -> None:
    async def processor(batch: list[int]) -> list[int]:
...
@pytest.mark.asyncio
async def test_process_batches_sequential() -> None:
    order = []
