...
class TestErrorMessages:
    ...
    def test_api_error_message(self):
        ...
        assert result == "GT Error: API returned error status. Status: 500, Status Text: Internal Server Error, Error: unexpected crash"

    def test_no_target_locale_error(self):
        result = no_target_locale_error("translate")
        assert result == "GT Error: Cannot call `translate` without a specified locale. Either pass a locale to the `translate` function or specify a targetLocale in the GT constructor."

    def test_no_source_locale_error(self):
        result = no_source_locale_error("translate")
        assert result == "GT Error: Cannot call `translate` without a specified locale. Either pass a locale to the `translate` function or specify a sourceLocale in the GT constructor."

    def test_no_project_id_error(self):
        result = no_project_id_error("translate")
        assert result == "GT Error: Cannot call `translate` without a specified project ID. Either pass a project ID to the `translate` function or specify a projectId in the GT constructor."

    def test_no_api_key_error(self):
        result = no_api_key_error("translate")
        assert result == "GT Error: Cannot call `translate` without a specified API key. Either pass an API key to the `translate` function or specify an apiKey in the GT constructor."
