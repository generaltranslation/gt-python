...
def test_declare_var_source_only():
...
def test_multiple_declare_vars():
...
def test_declare_var_with_user_variables():
...
def test_translated_with_declare_var():
...
def test_translated_multiple_declare_vars():
...
def test_translated_reordered_vars():
    """Translation can reorder _gt_ variables."""
    source = (
        f"{declare_var('Alice', name='user')} bought "
        f"{declare_var('apples', name='item')}"
    )
...
def test_fallback_retry_on_format_error():
...
def test_fallback_retry_preserves_user_vars():
    ...
    source = "Hello, {name}!"
    result = interpolate_message(
        bad_translation, {"$_fallback": source, "name": "Alice"}
    )
    assert result == "Hello, Alice!"


def test_fallback_retry_preserves_declare_vars():
