...
def test_parse_matches_reference(msg):
...
@pytest.mark.parametrize("msg", ALL_PARSE_CASES)
def test_parse_with_indices_matches_reference(msg):
...
def test_escaped_parse_matches_reference(msg):
...
class TestPrintAstModifications:
    ...
    def test_change_select_branch_content(self):
    ...
    def test_condense_indexed_select(self):
    ...
    def test_condense_multiple_indexed_selects(self):
        ast = Parser({"preserve_whitespace": True}).parse(
    ...
    def test_add_new_branch(self):
        ast = Parser({"preserve_whitespace": True}).parse(
            "{x, select, a {A} other {default}}"
        )
        ast[0]["options"]["b"] = ["B"]
        result = print_ast(ast)
        assert "b{B}" in result or "b {B}" in result

    def test_modify_plural_branch(self):
        ast = Parser({"preserve_whitespace": True}).parse(
            "{count, plural, one {# item} other {# items}}"
        )
...
class TestTokens:
    ...
    def test_tokens_for_plural(self):
        tokens = []
...
class TestErrors:
    ...
    def test_missing_placeholder_name(self):
        with pytest.raises(SyntaxError):
            Parser().parse("{}")

    def test_missing_submessages(self):
        with pytest.raises(SyntaxError):
            Parser().parse("{x, select}")

    def test_tokens_invalid_type(self):
        with pytest.raises(TypeError):
            Parser().parse("hello", "not a list")
